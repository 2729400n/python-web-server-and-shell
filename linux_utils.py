...
def format_passwd(r: "str|bytes"):
    ...
    user_info = {"username": _user_info_[0], "password": _user_info_[1], "uid": int(_user_info_[2]), "gid": int(
        _user_info_[3]), "uinfo": _user_info_[4].split(","), "home": _user_info_[5], "shell": _user_info_[6].strip("\n").strip("\r").strip("\n")}
...
global UIDS, USERS
UIDS, USERS = None,None # readusers()
...
def getuidByUser(name: str):
    for i in USERS:
        ...
        if(cur_user!=None):
            if(name == cur_user.get("username")):
                return int(USERS.get(i).get("uid"))


def getuserInfoByUser(name: str):
    return getuserInfoByUID(getuidByUser(name))
